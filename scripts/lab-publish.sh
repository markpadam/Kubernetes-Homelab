...
# Optional: also bind the Tailscale interface so the lab is reachable over the
# tailnet (a private WireGuard overlay) with no router/firewall changes. Purely
# additive — if Tailscale isn't installed, BIND_IPS stays just the LAN IP. Set
# LAB_NO_TAILSCALE=1 to skip even when Tailscale is present.
BIND_IPS=("$LAB_HOST_IP")
_TS_BIN="$(command -v tailscale 2>/dev/null || true)"
...
if [[ -z "${LAB_NO_TAILSCALE:-}" && -n "$_TS_BIN" ]]; then
  TS_IP=$("$_TS_BIN" ip -4 2>/dev/null | head -1)
  if [[ "$TS_IP" =~ ^100\. && "$TS_IP" != "$LAB_HOST_IP" ]]; then
    BIND_IPS+=("$TS_IP")
    log "Also publishing on Tailscale IP ${BOLD}${TS_IP}${RESET} (tailnet)"
  fi
fi
...
# (and the tailnet, if Tailscale is up). One socat per (bind IP, port):
# <BIND_IP>:PORT → 127.0.0.1:PORT.
SOCAT="${_SOCAT}"
BIND_IPS="${BIND_IPS[*]}"
PORTS="${PORTS[*]}"
LAB_USER="$(whoami)"
for ip in \$BIND_IPS; do
  for p in \$PORTS; do
    "\$SOCAT" TCP-LISTEN:\$p,bind=\$ip,fork,reuseaddr TCP:127.0.0.1:\$p &
  done
done
...
while true; do
  ...
  if [[ -n "\$_api" && "\$_api" != "\$_last_api" ]]; then
    for ip in \$BIND_IPS; do
      pkill -f "TCP-LISTEN:8443,bind=\$ip" 2>/dev/null
      "\$SOCAT" TCP-LISTEN:8443,bind=\$ip,fork,reuseaddr TCP:127.0.0.1:\$_api &
    done
    _last_api="\$_api"
  fi
  sleep 15
...
if sudo launchctl bootstrap system "$FORWARD_PLIST" 2>/dev/null; then
  success "Forwarder daemon running — ports {${PORTS[*]// /,}}+8443 on: ${BIND_IPS[*]} → 127.0.0.1"
else
