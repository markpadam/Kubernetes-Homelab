...
# 80/443 = NGINX ingress; 8200 = Vault; other ports gated by enabled features.
PORTS=(80 443 8200)
...
log "Installing port-forwarder daemon for: ${PORTS[*]}${_K8S_API_LOCAL:+ 8443→127.0.0.1:${_K8S_API_LOCAL}}"
...
# K8s API: expose on stable :8443 → actual dynamic minikube port.
${_K8S_API_LOCAL:+"\"${_SOCAT}\" TCP-LISTEN:8443,bind=${LAB_HOST_IP},fork,reuseaddr TCP:127.0.0.1:${_K8S_API_LOCAL} &"}
wait
WRAPPER
